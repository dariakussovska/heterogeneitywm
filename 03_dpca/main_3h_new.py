...
def run_dpca_distance_resampling_stratified(
    ...
    for it in range(n_iter):
        ...
        # FIX 1: More robust way to get explained variance
        try:
            if hasattr(dpca_early, "explained_variance_ratio_"):
                explvar_early.append(dpca_early.explained_variance_ratio_["s"])
            elif hasattr(dpca_early, "explained_variance_ratio"):
                explvar_early.append(dpca_early.explained_variance_ratio["s"])
            else:
                # Fallback: compute manually or skip
                print(f"Iteration {it}: No explained variance attribute found for early")
                explvar_early.append(np.zeros(len(selected_dpcs)))
        except Exception as e:
            print(f"Iteration {it}: Error getting early explained variance: {e}")
            explvar_early.append(np.zeros(len(selected_dpcs)))
        ...
        try:
            ...
            elif hasattr(dpca_late, "explained_variance_ratio"):
                explvar_late.append(dpca_late.explained_variance_ratio["s"])
            else:
                # Fallback: compute manually or skip
                print(f"Iteration {it}: No explained variance attribute found for late")
                explvar_late.append(np.zeros(len(selected_dpcs)))
        except Exception as e:
            print(f"Iteration {it}: Error getting late explained variance: {e}")
            explvar_late.append(np.zeros(len(selected_dpcs)))
    ...
    if kw_p < 0.05:
        ...
        # Find significant pairs for plotting
        for i in range(len(epochs)):
            for j in range(i+1, len(epochs)):
                if dunn_df.iloc[i, j] < 0.05:
                    significant_pairs.append((i, j, dunn_df.iloc[i, j]))
    else:
        print("Omnibus test not significant → skipping post-hoc.")

    # FIX 2: Add significance stars to the plots
    def add_significance_bars(ax, data_pairs, x_positions, y_max=None):
        """Add significance bars between conditions"""
        if y_max is None:
            y_max = max([max(d) for d in data_pairs]) * 1.1
            
        y_step = y_max * 0.05
        current_y = y_max
        
        for i, j, p_val in significant_pairs:
            # Only plot if both indices are in the current subplot
            if i < len(x_positions) and j < len(x_positions):
                ...
                # Draw the bar
                ax.plot([x1, x1, x2, x2], [current_y, current_y+y_step, current_y+y_step, current_y], 
                       lw=1.5, color='k')
                # Add stars
                ax.text((x1+x2)*0.5, current_y+y_step*1.2, stars, 
    ...
    # Add significance bars for train
    add_significance_bars(ax1, [(0, 1)], [1, 2], y_max=9)
    ...
    # Add significance bars for test
    add_significance_bars(ax2, [(0, 1), (0, 2), (1, 2)], [1, 2, 3], y_max=4.5)
    ...
    # FIX 1: Handle empty variance arrays
    try:
        explvar_early_arr = np.vstack(explvar_early)
        explvar_late_arr = np.vstack(explvar_late)
    except:
        print("Warning: Could not stack variance arrays, returning empty arrays")
        explvar_early_arr = np.array([])
        explvar_late_arr = np.array([])
...
measure_dict, kw_p, dunn_df, explvar_early, explvar_late = run_dpca_distance_resampling_stratified(
     ...
     selected_dpcs=[0, 1, 2, 3, 4],
     stim_indices=[0, 1, 2, 3, 4],
     savefig_path='./var_explained_late_early.eps',
)

# FIX 1: Plot variance explained with proper error handling
if explvar_early.size > 0:
    plt.figure(figsize=(10, 6))
    plt.boxplot(explvar_early[:, :5])
    plt.title('Explained variance: Early (first 5 components)')
    plt.ylabel('Explained Variance Ratio')
    plt.xlabel('dPC component')
    plt.show()
else:
    print("No variance explained data available for Early period")

if explvar_late.size > 0:
    plt.figure(figsize=(10, 6))
    plt.boxplot(explvar_late[:, :5])
    plt.title('Explained variance: Late (first 5 components)')
    plt.ylabel('Explained Variance Ratio')
    plt.xlabel('dPC component')
    plt.show()
else:
    print("No variance explained data available for Late period")
